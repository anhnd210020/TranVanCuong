...
class CombinedLossDWA(nn.Module):
    """Dynamic Weight Average (Liu et al. ICCV 2019)."""
    def __init__(self, alpha=0.25, gamma=2, T=2.0):
        super().__init__()
        self.focal = FocalLoss(alpha, gamma, reduction='mean')
        self.T     = T
        self.hist  = []  # lưu [(bce, focal)] mỗi epoch

    def forward(self, inputs, targets, epoch):
        bce   = F.binary_cross_entropy(inputs, targets, reduction='mean')
        foc   = self.focal(inputs, targets)
        if epoch < 2 or len(self.hist) < 2:
            w_bce = w_foc = 1.0
        else:
            r_bce, r_foc = self.hist[-1][0] / (self.hist[-2][0] + 1e-8), \
                           self.hist[-1][1] / (self.hist[-2][1] + 1e-8)
            ws = torch.softmax(torch.tensor([r_bce, r_foc], device=inputs.device)/self.T, dim=0)
            w_bce, w_foc = 2*ws[0], 2*ws[1]
        self.hist.append((bce.detach(), foc.detach()))
        return w_bce * bce + w_foc * foc
...
class CombinedLossUnc(nn.Module):
    ...
    def forward(self, inputs, targets):
        bce   = F.binary_cross_entropy(inputs, targets, reduction='mean')
        foc   = self.focal(inputs, targets)
        loss  = (0.5 * torch.exp(-self.log_sigma_bce)   * bce   + 0.5*self.log_sigma_bce) \
              + (0.5 * torch.exp(-self.log_sigma_focal) * foc   + 0.5*self.log_sigma_focal)
        return loss
...
class FraudGRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, day_index=None, hour_index=None, day_scale=2.0, hour_scale=2.0):
        ...
        # Lưu lại chỉ số và hệ số cho 2 features cần nhấn mạnh
    ...
    def forward(self, x):
        ...
        #     x[:, :, self.day_index] = x[:, :, self.day_index] * self.day_scale
        # if self.hour_index is not None:
        #     x[:, :, self.hour_index] = x[:, :, self.hour_index] * self.hour_scale
...
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        for X_batch, y_batch in progress_bar:
            ...
            # --- Tính loss adaptive nếu dùng DWA, còn lại gọi bình thường ---
            if isinstance(criterion, CombinedLossDWA):
                loss = criterion(outputs, y_batch, epoch)
            else:
        ...
        # Đánh giá trên train
        train_threshold, train_f1, train_auc, train_combined, train_acc, train_prec, train_rec = \
            evaluate_model(train_loader, model, device)
        print(
            f"Train Metrics - Best Threshold: {train_threshold:.2f}, "
            f"F1: {train_f1:.4f}, AUC: {train_auc:.4f}, "
            f"Combined: {train_combined:.4f}, "
            f"Accuracy: {train_acc:.4f}, Precision: {train_prec:.4f}, Recall: {train_rec:.4f}"
        )
        
        # Đánh giá trên test
        test_threshold, test_f1, test_auc, test_combined, test_acc, test_prec, test_rec = \
            evaluate_model(test_loader, model, device)
        print(
            f"Test Metrics  - Best Threshold: {test_threshold:.2f}, "
            f"F1: {test_f1:.4f}, AUC: {test_auc:.4f}, "
            f"Combined: {test_combined:.4f}, "
    ...
    print(
        f"Train Metrics - F1: {best_train_metrics[0]:.4f}, "
        f"AUC: {best_train_metrics[1]:.4f}, "
        f"Combined: {best_train_metrics[2]:.4f}"
    )
    print(
        f"Test Metrics  - F1: {best_test_metrics[0]:.4f}, "
        f"AUC: {best_test_metrics[1]:.4f}, "
        f"Combined: {best_test_metrics[2]:.4f}"
    )
...
criterion = CombinedLossDWA(alpha=0.25, gamma=2, T=2.0)
