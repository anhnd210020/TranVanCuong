...
class LearnableCombinedLoss(nn.Module):
    """
    Uncertainty–based weighting (Kendall et al., CVPR 2018):
    L = 0.5 * exp(-2*logσ1) * BCE + 0.5 * exp(-2*logσ2) * Focal + logσ1 + logσ2
    """
    def __init__(self, alpha=0.25, gamma=2.0, reduction='mean'):
        super().__init__()
        self.focal = FocalLoss(alpha=alpha, gamma=gamma, reduction='none')
        self.reduction = reduction
        # log(σ) để luôn >0 sau exp()
        self.log_sigma_bce   = nn.Parameter(torch.tensor(0.0))
        self.log_sigma_focal = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(self, inputs, targets):
        ...
        loss = (0.5 * torch.exp(-2 * self.log_sigma_bce)   * bce   +
                0.5 * torch.exp(-2 * self.log_sigma_focal) * focal +
                self.log_sigma_bce + self.log_sigma_focal)
        return loss
...
criterion = LearnableCombinedLoss(alpha=0.25, gamma=2.0, reduction='mean')
