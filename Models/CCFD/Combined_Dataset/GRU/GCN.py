...
class GCNLayer(nn.Module):
    def __init__(self, in_features, out_features, seq_len):
        ...
        # Build adjacency for a 1D chain + self-loops
        ...
        for i in range(seq_len):
            ...
            if i < seq_len - 1:
                A[i, i + 1] = 1.0
        # Normalize A_hat = D^{-1/2} (A + I) D^{-1/2}
        D = A.sum(dim=1)
    ...
    def forward(self, x):
        ...
        # Graph conv: X' = A_norm @ X
        x = torch.einsum('ij,bjf->bif', self.A_norm, x)
        # Feature-wise transform
...
class FraudGRU(nn.Module):
    def __init__(
        self, input_size, hidden_size, num_layers, seq_len,
        day_index=None, hour_index=None, day_scale=2.0, hour_scale=2.0
    ):
        super(FraudGRU, self).__init__()
        # Replace Shift-GCN with GCNLayer
        self.gcn = GCNLayer(input_size, hidden_size, seq_len)
        self.gru = nn.GRU(hidden_size, hidden_size, num_layers, batch_first=True)
        self.fc = nn.Linear(hidden_size, 1)
        self.sigmoid = nn.Sigmoid()
        # Indices & scaling for emphasis
    ...
    def forward(self, x):
        ...
        # Graph convolution over sequence
        x = self.gcn(x)  # → (batch, seq_len, hidden_size)
...
# Test Metrics  - Best Threshold: 0.50, F1: 0.8447, AUC: 0.9915, Combined: 0.9181, Accuracy: 0.9984, Precision: 0.8854, Recall: 0.8075