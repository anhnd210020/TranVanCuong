...
import sys, os
import time
sys.path.append("/home/ducanh/Financial Risk & Fraud Detection/venv/lib/python3.11/site-packages/ptflops")
...
feature_cols = [col for col in X_train.columns if col not in ['cc_num']]
print("Danh sách các feature được sử dụng để huấn luyện (total={}):".format(len(feature_cols)))
for idx, feat in enumerate(feature_cols):
    print(f" {idx+1:2d}. {feat}")

feature_names = [c for c in X_train_sc.columns if c != "cc_num"]
print("===== Input Channel Index → Feature Name =====")
...
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        for X_batch, y_batch in progress_bar:
            ...
            optimizer.zero_grad()
            outputs = model(X_batch).view(-1)   # (batch,)
            targets = y_batch.view(-1)          # (batch,)
...
def visualize_shift_gcn_mask(model, feature_names, node_index=0, save_path=None):

    raw_mask = model.shift_gcn.Feature_Mask
    mask_for_node = raw_mask.detach().cpu().numpy()[0, node_index, :]
    effective_mask = np.tanh(mask_for_node) + 1.0

    num_channels = effective_mask.shape[0]
    assert len(feature_names) == num_channels, "Số lượng tên feature không khớp số channel."

    print("===== Input Channel Index -> Feature Name =====")
    for idx, name in enumerate(feature_names):
        print(f"{idx:2d} ➜ {name}")
    print("===============================================")
    x_coords = np.arange(num_channels)

    plt.figure(figsize=(12, 4))
    plt.bar(x_coords, effective_mask, width=0.8)
    plt.xlabel("Input Feature")
    plt.ylabel("Mask Value (tanh + 1)")
    plt.title(f"Learned Feature_Mask for Shift_gcn (node {node_index})")
    plt.xticks(x_coords, feature_names, rotation=90, fontsize=7)
    plt.tight_layout()

    if save_path:
        plt.savefig(save_path, dpi=150)
        plt.close()
        print(f"Feature mask plot saved to: {save_path}")
    else:
        plt.show()
# =======================================================
...
# memory_size và input_size đã được định nghĩa ở trên
macs, params = get_model_complexity_info(
    model,
    (memory_size,  input_size),   # tương ứng (C, H, W) = (seq_len, 1, feat_dim)
    as_strings=False,
    print_per_layer_stat=False
)
if macs is None:
    print("⚠️ ptflops không đo được MACs, có thể do forward signature không tương thích.")
else:
    flops = 2 * macs                # xấp xỉ FLOPs = 2 * MACs
    tflops = flops / 1e12
    print(f"Model FLOPs  : {flops:,}")
    print(f"Model TFLOPs : {tflops:.4f} TFLOPs")

criterion = FocalLoss(alpha=0.25, gamma=2, reduction="mean").to(device)

print(f"Trainable parameters in FraudGRU:      {count_parameters(model)}")
print(f"Trainable parameters in FocalLoss:     {count_parameters(criterion)}")  # sẽ = 0
print(f"Total trainable parameters:           {count_parameters(model) + count_parameters(criterion)}")
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "fraudgru.pth")
    print("→ Saved model state_dict to fraudgru.pth")
    torch.save(model, "fraudgru_full.pth")
    print("→ Saved full model to fraudgru_full.pth")
    save_dir = "/home/ducanh/Financial Risk & Fraud Detection/Credit Card Fraud Detection/Models/CCFD/Combined_Dataset/DL models/GRU"
    save_path = f"{save_dir}/feature_mask.png"
    feature_names = [c for c in X_train_sc.columns if c != "cc_num"]
    # Visualize and save the mask plot:
    visualize_shift_gcn_mask(model, feature_names=feature_names, node_index=0, save_path=save_path)
