...
# 3. Định nghĩa Dataset và DataLoader
# =======================================================
...
# =======================================================
# 4. Định nghĩa mô hình FraudLSTM với num_nodes = 19
# =======================================================
class FraudLSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers,
                 num_nodes=19, day_index=None, hour_index=None,
                 day_scale=2.0, hour_scale=2.0):
        super(FraudLSTM, self).__init__()
        assert input_size % num_nodes == 0, "input_size must be divisible by num_nodes"
        self.num_nodes = num_nodes
        self.features_per_node = input_size // num_nodes

        self.shift_gcn = Shift_gcn(in_channels=self.features_per_node,
    ...
    def forward(self, x):
        ...
        x = x.contiguous().view(batch, seq, self.num_nodes, self.features_per_node)
        x = self.shift_gcn(x)
        x = x.mean(dim=3)
        x = x.permute(0, 2, 1).contiguous()  # ensure contiguous before LSTM
        out, _ = self.lstm(x)
        out = self.fc(out[:, -1, :])
...
# 6. Khởi tạo mô hình với num_nodes=4 và bắt đầu huấn luyện
# =======================================================
...
criterion = CombinedLoss(alpha=0.25, gamma=2,
