...
class FraudLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        out, (h_n, c_n) = self.lstm(x)            # now using LSTM
        out = self.fc(out[:, -1, :])
...
def train_model(model, train_loader, test_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Train Metrics - Best Threshold: {train_threshold:.2f}, "
              f"F1: {train_f1:.4f}, AUC: {train_auc:.4f}, Combined: {train_combined:.4f}, "
        ...
        print(f"Test Metrics  - Best Threshold: {test_threshold:.2f}, "
              f"F1: {test_f1:.4f}, AUC: {test_auc:.4f}, Combined: {test_combined:.4f}, "
    ...
    print(f"Train Metrics - F1: {best_train_metrics[0]:.4f}, "
          f"AUC: {best_train_metrics[1]:.4f}, Combined: {best_train_metrics[2]:.4f}")
    print(f"Test Metrics  - F1: {best_test_metrics[0]:.4f}, "
          f"AUC: {best_test_metrics[1]:.4f}, Combined: {best_test_metrics[2]:.4f}")
...
# instantiate your new LSTM-based model
